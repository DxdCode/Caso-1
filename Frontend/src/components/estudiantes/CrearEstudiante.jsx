...
import { Link } from "react-router-dom"; // ✅ Import correcto
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-3 bg-base">
        {menuItems.map((menu, idx) => (
          <ul key={idx} className="space-y-1 text-sec">
            <div className="font-bold truncate pl-2">{open && menu.label}</div>
            {menu.actions.map((action, i) => (
              <li
                key={i}
                className={`flex items-center ${open ? "justify-start" : "justify-center"} gap-2 p-2 rounded-md hover:rounded-md hover:bg-[#8B5CF6] hover:text-white cursor-pointer transition-colors duration-300`}
                title={!open ? `${action.name} ${menu.label.slice(0, -1)}` : ""}
              >
                <Link to={action.path} className="flex items-center gap-2 w-full">
                  {action.icon}
